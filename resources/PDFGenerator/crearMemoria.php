<?php
    ...
    else if(isset($_GET["demo"]))
    {
        ...
        cerradores.nombre AS nombre_cerrador
        FROM demos 
        JOIN clientes ON demos.cliente = clientes.id_cliente 
        JOIN colaboradores AS cerradores ON clientes.cerrador = cerradores.id_colaborador
        WHERE id_demo = ?";
        $options = array($id_demo);
        $query = $db->prepare($queryString);
        $query -> execute($options);
        if($query->rowCount() != 0)
        {
            ...
            $equipo = $data['equipo'];
            $fecha_inicio = $data['registro'];
            $periodo = $data['periodo'];
            $nombre_cerrador = $data['nombre_cerrador'];
            $fecha_termino = date('Y-m-d',strtotime("+$periodo days",strtotime($fecha_inicio)));
            ...
            require($root.'resources/PDFGenerator/genera_memoria_demo.php');
        }
        else
        {
            $pageTitle = "Error";
